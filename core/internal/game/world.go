...
type World struct {
	...
	// Enhanced power-up system (SIMPLIFIED: only 3 power-ups)
	ActivePowerUp  PowerUpType     // Current active power-up type
	IsSpeedBoosted bool            // Runner has speed boost
	IsMagnetActive bool            // Pellet magnet active
	Difficulty     DifficultyLevel // Game difficulty
	// REMOVED: IsInvisible, AreChasersFrozen - simplified power-up system

	// State snapshot for reconnection (Sprint 1 critical fix)
...
func NewWorldState() *World {
	...
	// Initialize snapshot manager for reconnection support
	world.SnapshotManager = NewSnapshotManager(world)
	world.SnapshotManager.Start()
...
// TryEatPellet attempts to claim and eat a pellet (Item #19 - collision claim protocol)
// Returns true if successful, false if already eaten by another player
func (w *World) TryEatPellet(pelletX, pelletY float64, playerID string) bool {
	// Use atomic add-if-not-exists
	if w.PelletsCoordEaten.Has(pelletX, pelletY) {
		return false // Already eaten
	}

	// Try to add - if another goroutine added it first, this will fail
	if !w.PelletsCoordEaten.TryAdd(pelletX, pelletY) {
		return false // Race condition - another player claimed it first
	}
...
func (w *World) EatPowerUp(powerUpX, powerUpY float64) {
	...
	settings := PowerUpSettings[powerUpType]

	// If already powered up with same type, extend duration
	if w.IsPoweredUp && w.ActivePowerUp == powerUpType {
		w.PowerUpEndTime = time.Now().Add(settings.Duration)
	...
	// Clear previous power-up effects
	w.clearPowerUpEffectsUnlocked()

	w.PowerUpsCoordsEaten.Add(powerUpX, powerUpY)
	w.IsPoweredUp = true
	w.ActivePowerUp = powerUpType
	w.PowerUpEndTime = time.Now().Add(settings.Duration)

	// Apply power-up specific effects
	w.applyPowerUpEffectsUnlocked(powerUpType)
	...
	if w.broadcastFunc != nil {
		msg, _ := json.Marshal(map[string]interface{}{
			...
			"x":           powerUpX,
			"y":           powerUpY,
			"duration":    int(settings.Duration.Seconds()),
			"powerUpType": int(powerUpType),
			"powerUpName": getPowerUpName(powerUpType),
		})
		w.broadcastFunc(msg)
	}
	log.Info().Float64("x", powerUpX).Float64("y", powerUpY).Int("type", int(powerUpType)).Msg("Power-up activated")

	// Start power-up timer (skip for instant effects like teleport)
	if settings.Duration > 0 {
		w.startPowerUpTimerUnlocked()
	}
}

// applyPowerUpEffectsUnlocked applies effects for a power-up type (caller holds lock)
// SIMPLIFIED: Only 3 power-ups now (Classic, Speed, Magnet)
func (w *World) applyPowerUpEffectsUnlocked(powerUpType PowerUpType) {
	switch powerUpType {
	case PowerUpClassic:
		// Classic power-up - can eat chasers (handled elsewhere)
		// Set bots to panic mode
		if w.BotManager != nil {
			for _, bot := range w.BotManager.GetBots() {
				if bot.PlayerEntity.SpriteType != Runner {
					bot.SetPanicMode(true)
				}
			}
		}
	case PowerUpSpeed:
		w.IsSpeedBoosted = true
	case PowerUpMagnet:
		w.IsMagnetActive = true
	}
}

// clearPowerUpEffectsUnlocked clears all active power-up effects (caller holds lock)
// SIMPLIFIED: Only 3 power-ups now
func (w *World) clearPowerUpEffectsUnlocked() {
	w.IsPoweredUp = false
	w.IsSpeedBoosted = false
	w.IsMagnetActive = false

	// Reset bot panic mode
	if w.BotManager != nil {
		for _, bot := range w.BotManager.GetBots() {
			bot.SetPanicMode(false)
		}
	}
}

// getPowerUpName returns the display name for a power-up type
// SIMPLIFIED: Only 3 power-ups now
func getPowerUpName(powerUpType PowerUpType) string {
	names := map[PowerUpType]string{
		PowerUpClassic: "Power Mode",
		PowerUpSpeed:   "Speed Boost",
		PowerUpMagnet:  "Magnet",
	}
	if name, ok := names[powerUpType]; ok {
		return name
	}
	return "Unknown"
}
...
func (w *World) startPowerUpTimerUnlocked() {
	...
	// Get duration from current power-up settings
	duration := PowerUpDuration
	if settings, ok := PowerUpSettings[w.ActivePowerUp]; ok {
		duration = settings.Duration
	}

	w.PowerUpTimer = time.AfterFunc(duration, func() {
		w.worldLock.Lock()
...
func (w *World) StartDynamicSystems(broadcastFunc func(msgType string, data interface{})) {
	...
	// Set MazeData reference for dynamic wall collision updates
	if w.MazeData != nil {
		w.DynamicWorld.SetMazeDataRef(w.MazeData)
	}

	// Provide maze data to entity manager for pathfinding
	if w.MazeData != nil {
		mazeGrid := make([][]int, w.MazeHeight)
		for y := 0; y < w.MazeHeight; y++ {
			mazeGrid[y] = make([]int, w.MazeWidth)
			for x := 0; x < w.MazeWidth; x++ {
				if y < len(w.MazeData.Walls) && x < len(w.MazeData.Walls[y]) && w.MazeData.Walls[y][x] {
					mazeGrid[y][x] = 1 // Wall
				} else {
					mazeGrid[y][x] = 0 // Walkable
				}
			}
		}
		w.EntityManager.SetMazeData(mazeGrid)
	}
