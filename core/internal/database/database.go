...
import (
	...
	"github.com/frank2889/mazechase/internal/monetization"
...
func InitDB() *gorm.DB {
	...
	// Migrate monetization tables
	monetizationRepo := monetization.NewMonetizationRepository(db)
	if err := monetizationRepo.AutoMigrate(); err != nil {
		log.Fatal().Err(err).Msg("failed to migrate monetization tables")
	}
