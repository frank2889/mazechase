...
- **Third-person camera** volgt de speler bal van achteren
- **Speler is een STUITERENDE BAL** - beweging via bounce physics
- **Space/Tap om te bouncen** - timing-based controls
- **Pellets op random locaties** - minder pellets dan traditionele maze games
...
```typescript
// Camera volgt de speler bal van achteren
const camera = new BABYLON.FollowCamera('followCam', 
    new BABYLON.Vector3(0, 5, -8), // Achter en boven de bal
    scene
);
camera.radius = 8;          // Afstand achter bal
camera.heightOffset = 5;    // Hoogte boven bal
camera.rotationOffset = 0;  // Recht achter
camera.cameraAcceleration = 0.05;  // Smooth following
camera.maxCameraSpeed = 10;
camera.lockedTarget = playerBall;  // Volg de speler
```

### Camera Behavior
| State | Behavior |
|-------|----------|
| Idle | Close to ball, steady |
| Moving | Pulls back slightly |
| Fast | Pulls back more, wider FOV |
| Bouncing | Subtle vertical movement sync |
