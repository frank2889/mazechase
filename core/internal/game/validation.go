...
func (v *InputValidator) ValidateSpriteType(spriteType string) error {
	validSprites := map[string]bool{
		"runner": true, "ch0": true, "ch1": true, "ch2": true,
	}
...
// sanitizeString removes potentially dangerous characters and problematic Unicode
func sanitizeString(s string) string {
	...
	// Filter out problematic Unicode characters that can corrupt UI
	...
	for _, r := range s {
		...
		// Skip Unicode replacement character and BOM
		if r == 0xFFFD || r == 0xFEFF {
			continue
		}
		// Skip right-to-left override and other directional formatting
		if r >= 0x200E && r <= 0x200F { // LTR/RTL marks
			continue
		}
		if r >= 0x202A && r <= 0x202E { // Directional formatting
			continue
		}
		if r >= 0x2066 && r <= 0x2069 { // Isolates
			continue
		}
		// Skip zero-width characters that can cause rendering issues
		if r == 0x200B || r == 0x200C || r == 0x200D { // ZWSP, ZWNJ, ZWJ
			continue
		}
		result.WriteRune(r)
	}
