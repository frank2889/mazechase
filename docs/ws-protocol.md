...
Alle communicatie tussen client en server verloopt via WebSocket met JSON messages. Dit document beschrijft het message protocol.

### Core Gameplay Note
- **Speler is een stuiterende bal** met physics-based movement
- **Third-person camera** volgt de bal
- **Space/Tap om te bouncen** - server ontvangt bounce events, niet directional movement
...
### Player Bounce (Core Mechanic)

```json
{
    "type": "player_bounce",
    "payload": {
        "charge": 0.0-1.0,     // Charge level (0=tap, 1=full hold)
        "direction": {         // Lean direction influence
            "x": -1.0 to 1.0,
            "y": -1.0 to 1.0
        },
...
### Legacy: Player Movement (deprecated - use player_bounce)
