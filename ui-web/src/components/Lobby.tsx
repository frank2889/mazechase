...
import {DIFFICULTY_LEVELS, type DifficultyLevel, getDifficulty, setDifficulty as saveDifficulty} from "../lib/game/difficulty.ts";
import {
    Gamepad2, Trophy, Link2, Plus, ClipboardList, Play, Check, 
    Copy, Trash2, Users, Zap, Target, Flag, Swords, LogOut, X
} from 'lucide-solid';
...
const LobbyComponent: Component = () => {
    ...
    const [selectedDifficulty, setSelectedDifficulty] = createSignal<DifficultyLevel>(getDifficulty());
    ...
    const handleJoinLobby = (lobbyId: string, mode: GameMode = selectedMode()) => {
        window.location.assign(`/game?lobby=${lobbyId}&mode=${mode}&difficulty=${selectedDifficulty()}`);
    };
    ...
    return (
        <div class="min-h-screen bg-gradient-to-b from-slate-900 via-purple-950 to-slate-900">
            ...
            <div class="px-6 pb-6">
                ...
                <div class="max-w-md mx-auto mb-6">
                    <button
                        onClick={async () => {
                            ...
                            const {val, err} = await addLobby(`Solo-${currentUser()}-${Date.now()}`);
                            if (err) {
                                showSnackbar(`Fout: ${err}`, 'error');
                                return;
                            }
                            // Use the returned lobby ID directly
                            if (val?.lobbyId) {
                                window.location.assign(`/game?lobby=${val.lobbyId}&mode=${selectedMode()}&difficulty=${selectedDifficulty()}&single=true`);
                            } else {
                                showSnackbar('Kon lobby niet aanmaken', 'error');
                            }
                        }}
                        ...
                        <For each={Object.values(GAME_MODES)}>
                            {(mode) => (
                                ...
                                >
                                    ...
                                    <div class="text-gray-400 text-xs mt-1">{mode.description}</div>
                                </button>
                            )}
                        </For>
                    </div>
                ...
                {/* Difficulty Selection - AI Tester Suggestion: Emma, Tim, Sandra, Peter, Grandma Mei */}
                <div class="max-w-2xl mx-auto mb-6">
                    <h3 class="text-gray-300 text-xl font-semibold mb-3 text-center flex items-center justify-center gap-2">
                        <Target class="w-5 h-5 text-cyan-400" /> Kies Moeilijkheidsgraad
                    </h3>
                    <div class="grid grid-cols-3 gap-3">
                        <For each={Object.values(DIFFICULTY_LEVELS)}>
                            {(diff) => (
                                <button
                                    onClick={() => handleDifficultyChange(diff.id)}
                                    class={`p-4 rounded-lg border-2 transition-all ${
                                        selectedDifficulty() === diff.id
                                            ? `border-cyan-400 bg-gradient-to-br ${diff.color} scale-105`
                                            : 'border-slate-600 bg-slate-800/50 hover:border-slate-500'
                                    }`}
                                >
                                    <div class="text-2xl mb-1">{diff.icon}</div>
                                    <div class="text-white font-bold">{diff.nameNL}</div>
                                    <div class="text-gray-300 text-xs mt-1">{diff.description}</div>
                                </button>
                            )}
                        </For>
                    </div>
                ...
                <div class="max-w-md mx-auto mb-6">
                    <div class="p-4 rounded-lg shadow-lg bg-cyan-900/30 backdrop-blur-md border border-cyan-500/30">
                        <div class="text-gray-300 text-lg font-semibold mb-2 flex items-center gap-2">
                            <Link2 class="w-5 h-5 text-cyan-400" /> Join by Code or Name
                        </div>
                    ...
                    <div class="p-6 rounded-lg shadow-lg bg-slate-800/50 backdrop-blur-md border border-slate-700/50">
                        <div class="flex gap-3 items-center">
                            <div class="text-gray-300 text-xl font-semibold flex items-center gap-2">
                                <Plus class="w-5 h-5 text-green-400" /> Create Lobby
                            </div>
                            <input
                                type="text"
                ...
                >
                    <h3 class="text-gray-300 text-xl font-semibold mb-3 text-center flex items-center justify-center gap-2">
                        <ClipboardList class="w-5 h-5 text-purple-400" /> Active Lobbies
                    </h3>
...
const LobbyCard: Component<LobbyCardProps> = (props) => {
    ...
    const copyShareLink = async () => {
        ...
        const shareText = `Join my MazeChase game!\nLobby: ${props.lobby.lobbyName}\nLink: ${getShareLink()}`;
        try {
            await navigator.clipboard.writeText(shareText);
            setCopied(true);
            props.showSnackbar?.(`Link copied! (${mode.name})`, 'success');
        ...
        >
            ...
            <div class="flex flex-col gap-2">
                <div class="flex justify-between items-center">
                    ...
                    >
                        {copied() ? <><Check class="w-4 h-4" /> OK</> : <><Copy class="w-4 h-4" /> Share</>}
                    </button>
                </div>
