...
type Bot struct {
	...
	// New unpredictability features
	Unpredictability   float64   // 0.0 to 1.0 - random behavior chance
	MoodTimer          int       // Ticks until next mood change
	CurrentMood        BotMood   // Current behavioral mood
	SpeedMultiplier    float64   // Speed variation
	PanicMode          bool      // When runner is powered up
	LastStrategySwitch time.Time // When strategy was last changed
}

// BotMood represents temporary behavioral states
type BotMood int

const (
	MoodNormal      BotMood = iota
	MoodAggressive          // Extra chase focus
	MoodCautious            // Keep distance, watch
	MoodErratic             // Unpredictable movements
	MoodCoordinated         // Work with other bots
	MoodHunting             // AI Tester suggestion: Stalking mode - slow approach then burst
	MoodPlayful             // AI Tester suggestion: Taunting, comes close then backs off
)
...
type BotManager struct {
	...
	difficulty DifficultySettingsData // AI Tester suggestion: adjustable difficulty
}
...
// SetDifficultySettings applies difficulty settings to bot behavior (AI Tester suggestion)
func (bm *BotManager) SetDifficultySettings(settings DifficultySettingsData) {
	bm.mutex.Lock()
	defer bm.mutex.Unlock()
	bm.difficulty = settings
	// Apply to existing bots
	for _, bot := range bm.bots {
		bot.SpeedMultiplier = settings.BotSpeedMultiplier
		bot.AggressionLevel = settings.BotAggressionBase
		bot.Unpredictability = settings.BotUnpredictability
	}
}

// GetDifficulty returns current difficulty settings
func (bm *BotManager) GetDifficulty() DifficultySettingsData {
	bm.mutex.Lock()
	defer bm.mutex.Unlock()
	return bm.difficulty
}
...
func NewBotManager(world *World, broadcastFunc func([]byte) error) *BotManager {
	return &BotManager{
		...
		difficulty: GetDifficultySettingsData(DifficultyMedium), // Default to medium
	}
}
...
func (bm *BotManager) createBotUnlocked(index int) *Bot {
	...
	// Randomize unpredictability based on bot index
	unpredictability := 0.2 + float64(index%4)*0.15 // 0.2 to 0.65
	speedMultiplier := 0.9 + rand.Float64()*0.3     // 0.9 to 1.2
	...
	bot := &Bot{
		...
		MoodTimer:          50 + rand.Intn(100),
		CurrentMood:        MoodNormal,
		SpeedMultiplier:    speedMultiplier,
		PanicMode:          false,
		LastStrategySwitch: time.Now(),
	}
	...
	log.Info().
		...
		Float64("unpredictability", unpredictability).
		Msg("Bot created with enhanced AI")
...
func (b *Bot) Start(broadcast func([]byte) error) {
	...
	for {
		...
		case <-ticker.C:
			...
			// Update mood periodically for unpredictable behavior
			b.updateMood(tickCounter)

			// Randomly switch strategy for less predictability
			b.maybeChangeStrategy()

			// Change direction based on mood and strategy
			directionChangeCounter++
			dirChangeThreshold := b.getDirectionChangeThreshold()
			randomChance := b.getRandomDirectionChance()

			if directionChangeCounter > dirChangeThreshold || rand.Float32() < randomChance {
				currentDir = b.chooseNewDirection(currentDir)
				directionChangeCounter = 0
			}

			// Calculate new position based on direction with speed variation
			baseSpeed := PlayerSpeed * 0.2 * 0.001 * 200
			speed := baseSpeed * b.SpeedMultiplier

			// Panic mode when runner is powered up - move faster but erratically
			if b.PanicMode {
				speed *= 1.3
			}
...
func (b *Bot) chooseNewDirection(currentDir string) string {
	...
	for _, dir := range directions {
		...
		if b.World.MazeData == nil || b.World.MazeData.CanMoveTo(b.PlayerEntity.X, b.PlayerEntity.Y, testX, testY) {
			...
			// Apply mood-based modifiers for unpredictability
			switch b.CurrentMood {
			case MoodErratic:
				// Very unpredictable - add lots of randomness
				score += (rand.Float64() - 0.5) * 200
			case MoodAggressive:
				// More focused on target
				score *= 1.5
			case MoodCautious:
				// Add penalty for getting too close
				if distToTarget < 150 {
					score -= 100
				}
			case MoodCoordinated:
				// Try to approach from different angle than last time
				score += rand.Float64() * 50
			case MoodHunting:
				// AI Tester suggestion: Stalking behavior
				// Stay at medium distance, then close in
				if distToTarget > 200 {
					score *= 1.3 // Approach
				} else if distToTarget < 100 {
					score *= 0.7 // Hold back a bit
				}
			case MoodPlayful:
				// AI Tester suggestion: Taunting behavior
				// Come close then back off - oscillating
				if distToTarget < 80 {
					score = distToTarget * 0.3 // Move away
				} else if distToTarget > 200 {
					score *= 1.4 // Come closer
				}
			}

			// Panic mode - run away from runner
			if b.PanicMode && isChaser {
				score = distToTarget * 0.5 // Prefer moving AWAY
			}

			// Add unpredictability factor
			score += (rand.Float64() - 0.5) * 80 * b.Unpredictability
	...
	// Pick direction - usually best score, but sometimes second best for unpredictability
	// Sort by score
	for i := 0; i < len(validDirs)-1; i++ {
		for j := i + 1; j < len(validDirs); j++ {
			if validDirs[j].score > validDirs[i].score {
				validDirs[i], validDirs[j] = validDirs[j], validDirs[i]
			}
		}
	}

	// Sometimes pick second best for unpredictability
	if len(validDirs) > 1 && rand.Float64() < b.Unpredictability*0.3 {
		return validDirs[1].dir
	}
...
// updateMood updates the bot's behavioral mood for unpredictability
func (b *Bot) updateMood(tickCounter int) {
	b.MoodTimer--
	if b.MoodTimer <= 0 {
		// Change mood randomly - AI Tester suggestion: more variety
		moods := []BotMood{MoodNormal, MoodAggressive, MoodCautious, MoodErratic, MoodCoordinated, MoodHunting, MoodPlayful}
		weights := []float64{0.2, 0.2, 0.12, 0.13, 0.13, 0.12, 0.1}
		...
		for i, weight := range weights {
			cumulative += weight
			if r < cumulative {
				b.CurrentMood = moods[i]
				break
			}
		}

		// Random mood duration
		b.MoodTimer = 30 + rand.Intn(70) // 6-14 seconds at 200ms ticks

		// Adjust speed multiplier based on mood
		switch b.CurrentMood {
		case MoodAggressive:
			b.SpeedMultiplier = 1.1 + rand.Float64()*0.2
		case MoodCautious:
			b.SpeedMultiplier = 0.7 + rand.Float64()*0.2
		case MoodErratic:
			b.SpeedMultiplier = 0.8 + rand.Float64()*0.4
		case MoodHunting:
			b.SpeedMultiplier = 0.6 + rand.Float64()*0.2 // Slow stalk
		case MoodPlayful:
			b.SpeedMultiplier = 1.0 + rand.Float64()*0.3 // Normal to fast
		default:
			b.SpeedMultiplier = 0.9 + rand.Float64()*0.2
		}
	}

	// AI Tester suggestion: Hunting mode burst - suddenly speed up when close
	if b.CurrentMood == MoodHunting && tickCounter%5 == 0 {
		runnerX, runnerY := b.getRunnerPosition()
		dx := b.PlayerEntity.X - runnerX
		dy := b.PlayerEntity.Y - runnerY
		dist := math.Sqrt(dx*dx + dy*dy)
		if dist < 150 { // Close to runner - BURST!
			b.SpeedMultiplier = 1.4
		}
	}
}

// maybeChangeStrategy randomly changes strategy for unpredictability
func (b *Bot) maybeChangeStrategy() {
	// Only change strategy occasionally
	if time.Since(b.LastStrategySwitch) < 10*time.Second {
		return
	}

	// Random chance based on unpredictability
	if rand.Float64() > b.Unpredictability {
		return
	}
	...
	if newStrategy != b.Strategy {
		b.Strategy = newStrategy
		b.LastStrategySwitch = time.Now()
	}
}

// getDirectionChangeThreshold returns how often to change direction based on mood
func (b *Bot) getDirectionChangeThreshold() int {
	baseThreshold := 10
	switch b.CurrentMood {
	case MoodAggressive:
		return baseThreshold + 5 // More focused, less direction changes
	case MoodErratic:
		return baseThreshold - 5 // Very unpredictable
	case MoodCautious:
		return baseThreshold + 10 // Very deliberate
	case MoodHunting:
		return baseThreshold + 8 // Stalking - stay on course
	case MoodPlayful:
		return baseThreshold - 3 // More direction changes for taunting
	default:
		return baseThreshold + rand.Intn(10)
	}
}

// getRandomDirectionChance returns probability of random direction change
func (b *Bot) getRandomDirectionChance() float32 {
	baseChance := float32(0.1)
	switch b.CurrentMood {
	case MoodErratic:
		return 0.25 // Very random
	case MoodAggressive:
		return 0.05 // Focused
	case MoodCoordinated:
		return 0.08
	case MoodHunting:
		return 0.03 // Very focused stalking
	case MoodPlayful:
		return 0.20 // Unpredictable taunting
	default:
		return baseChance + float32(b.Unpredictability)*0.1
	}
}

// SetPanicMode sets whether the bot should be in panic mode (when runner has power-up)
func (b *Bot) SetPanicMode(panic bool) {
	b.mutex.Lock()
	defer b.mutex.Unlock()
	b.PanicMode = panic
	if panic {
		b.CurrentMood = MoodErratic // Run away erratically
