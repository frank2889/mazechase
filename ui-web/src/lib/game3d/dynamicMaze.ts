...
// Collision mesh callback type - called when walls change for physics sync
type CollisionCallback = (action: 'add' | 'remove' | 'move', x: number, z: number, toX?: number, toZ?: number) => void;
...
export class DynamicMaze {
    ...
    private onCollisionChange: CollisionCallback | null = null;
    ...
    /**
     * Set callback for collision mesh updates (Item #22)
     */
    public setCollisionCallback(callback: CollisionCallback): void {
        this.onCollisionChange = callback;
    }
    
    /**
     * Enable/disable audio cues for wall changes
     */
    ...
    /**
     * Play audio cue for wall changes (Item #26)
     */
    private playWallAudio(action: 'add' | 'remove' | 'move'): void {
        if (!this.audioEnabled) return;
        
        import('../game/audio').then(({ playSound }) => {
            // Use appropriate sounds based on action
            switch (action) {
                case 'add':
                    playSound('power_pellet'); // Rising sound for wall appearing
                    break;
                case 'remove':
                    playSound('chomp'); // Eating sound for wall disappearing
                    break;
                case 'move':
                    playSound('siren'); // Alert sound for moving wall
                    break;
            }
        }).catch(() => {
    ...
    private addWall(x: number, z: number, duration: number): void {
        ...
        wall.checkCollisions = true; // Enable collision from start
        
        this.wallMeshes.set(key, wall);
        this.animatingWalls.add(key);
        
        // Play audio cue (Item #26)
        this.playWallAudio('add');
        
        // Notify collision system immediately (Item #22)
        if (this.onCollisionChange) {
            this.onCollisionChange('add', x, z);
        }
    ...
    private removeWall(x: number, z: number, duration: number): void {
        ...
        // Play audio cue (Item #26)
        this.playWallAudio('remove');
        
        // Disable collision immediately when wall starts removing (Item #22)
        wall.checkCollisions = false;
        if (this.onCollisionChange) {
            this.onCollisionChange('remove', x, z);
        }
    ...
    private moveWall(
        ...
        // Play audio cue (Item #26)
        this.playWallAudio('move');
        
        // Notify collision system of wall move (Item #22)
        if (this.onCollisionChange) {
            this.onCollisionChange('move', fromX, fromZ, toX, toZ);
        }
    ...
    /**
     * Get unique key for wall position (Item #27: consistent floor rounding)
     */
    private getWallKey(x: number, z: number): string {
        // Always use floor for consistent key generation
